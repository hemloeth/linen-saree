                        ...
                        >
                            <motion.div
                                ...
                                className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                                    isError ? "bg-destructive/15 text-destructive" : "bg-green-500 text-white"
                                }`}
                            >
                                {isError ? (
                                    <AlertCircle className="w-4.5 h-4.5" strokeWidth={2.5} />
                                ) : (
                                    <Check className="w-4 h-4 text-white" strokeWidth={3} />
                                )}
                            </motion.div>
                            <div className="flex-1 min-w-0">
                                <p className={`text-sm font-semibold ${isError ? "text-destructive" : "text-green-600 dark:text-green-400"}`}>
                                    {toast.title}
                                </p>
